...
static constexpr int      COORD_PANEL_CLIENT_W = 560;
static constexpr int      COORD_PANEL_CLIENT_H = 76;
static constexpr int      COORD_PANEL_VISIBLE_W = 560;
...
static bool LoadZoneActions(POINT start, POINT end, std::string* actions, std::wstring* status) {
    ...
    if (body.empty()) {
        if (status) *status = L"Missing or empty Tasket example template: Click Drag Zone Flow.scht";
        return false;
    }
    ...
    if (!ReplaceNextCursorMovementPoint(body, 0, start, &nextFrom) ||
        ...
        if (status) *status = L"Could not update coordinate points in Click Drag Zone Flow.scht template.";
        return false;
    }

    if (!ExtractTasketActionsBlock(body, actions)) {
        if (status) *status = L"Could not read actions from Click Drag Zone Flow.scht.";
        return false;
    }
    return true;
}
...
static bool LoadCursorMoveActions(POINT target, std::string* actions, std::wstring* status) {
    ...
    if (body.empty()) {
        if (status) *status = L"Missing or empty Tasket example template: Move cursor example for codex.scht";
        return false;
    }

    if (!ReplaceNextCursorMovementPoint(body, 0, target, nullptr)) {
        if (status) *status = L"Could not update coordinate point in Move cursor example for codex.scht.";
        return false;
    }

    if (!ExtractTasketActionsBlock(body, actions)) {
        if (status) *status = L"Could not read actions from Move cursor example for codex.scht.";
        return false;
    }
    return true;
}
