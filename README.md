...
```powershell
cmake --build .\build-vs --config Release --target CursorOverlay
start .\build-vs\bin\Release\CursorOverlay.exe

# (kill it when done, it's portable, with no listeners, nor mouse click hooks):
Get-Process CursorOverlay -ErrorAction SilentlyContinue | Stop-Process -Force
```

*builds json for you, and has a dance-dance revolution style keyboard**

..showing recent presses (of course), native target system, live coordinate tracking, plus native keybind combinations across all macro programs. Extremely useful for vision, playback, or another framework that relies on recording macros at scale.

It's 200kb 💀 why did this framework not exist beforehand? 
> <1mb for dance-dance revolution keyboard? sign me up

No release build included, since using the latest Visual Studio as maker is probably the best route for this, and I don't want to keep up releases. Fully AGPL.

Upcoming:
- toggleable circle or some type of bbox layering system - maybe radian offsets?
